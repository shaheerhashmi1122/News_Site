            ...
            <div className="container">
                <div className="row">
                    <div className="col-md-8 col-sm-10 order-md-1 order-2 read-part ">
                        ...
                        {sports.map((sports_news)=>(
                        <div className="read-section w-100 py-4 px-2">
                            <img
                                className="w-50"
                                src={`/news/${sports_news?.image}`}
                                alt=""
                            />

                            <h2>{sports_news?.heading}</h2>
                            <p>
                            {sports_news?.description}
                            </p>
                            <a className="btn btn-primary" href={route("user.read", sports_news.id)}>Read More</a>
                        </div>
                    ))}
                    </div>
